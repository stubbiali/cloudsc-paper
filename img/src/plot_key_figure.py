...
from common import Bar, IMG_DIR
from plot_runtime_lumi_1 import get_data as get_data_lumi
...
def fill_ax_00(ax: plt.Axes, data: list[Bar], text: str) -> list[Any]:
    ylim = (0, 1.2 * data[-2].y)
    yoff = 0.02 * ylim[1]
...
def fill_ax_x0(ax: plt.Axes, data: list[Bar], text: str) -> None:
    ylim = (0, 1.2 * data[-2].y)
    yoff = 0.02 * ylim[1]
...
def fill_ax_x1(ax: plt.Axes, data: list[Bar], text: str) -> None:
    ylim = (0, 1.2 * data[-2].y)
    yoff = 0.02 * ylim[1]
...
def main(show: bool, save: bool) -> None:
    ...
    handles = fill_ax_00(ax_00, data_00, text="MeluXina\nFP64")
    ax_00.set_title(
        "Runtime (in ms) for CLOUDSC", loc="center", fontsize=16, fontweight="bold", pad=20
    )
    fill_ax_x0(ax_10, data_10, text="LUMI\nFP64")
    fill_ax_x1(ax_01, data_01, text="MeluXina\nFP64")
    ax_01.set_title(
    ...
    fill_ax_x1(ax_11, data_11, text="LUMI\nFP64")
    ...
    if save:
        ...
        fig.savefig(os.path.join(IMG_DIR, "final/performance_mlux_lumi.pdf"))
    if show:
        plt.show()
